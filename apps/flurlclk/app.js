...
let onTap = function() {
  // a timer is already set => double tap
  if (tapTimeout) {
    clearTimeout(tapTimeout);
    tapTimeout = undefined;
    doAction(settings.doubleTapAction);
  } else {
...
Bangle.setUI({
  ...
  touch: onTap,
  swipe: (dirH, dirV) => {
    if (dirH == 1) doAction(settings.swipeRightAction);
    else if (dirH == -1) doAction(settings.swipeLeftAction);
    else if (dirV == 1) doAction(settings.swipeDownAction);
    else if (dirV == -1) doAction(settings.swipeUpAction);
  }
});
// Load widgets
Bangle.loadWidgets();
